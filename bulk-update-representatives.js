...
// SHERLOCK v25.0: اسکریپت جامع بروزرسانی اطلاعات نمایندگان
// بدون آسیب به ساختار اتوماسیون و کوپلینگ‌ها

const fs = require('fs');
const path = require('path');

// داده‌های لیست نمایندگان شما
    ...
    for (const line of lines) {
        ...
        if (parts.length >= 4) {
            parsed.push({
                ...
                sales_associate: parts[1],
                representative_name: parts[2],
                telegram_id: parts[3]
            });
        }
    }
...
// تابع اصلی بروزرسانی
async function updateRepresentativesData() {
    try {
        console.log('🚀 SHERLOCK v25.0: شروع بروزرسانی جامع اطلاعات نمایندگان...');
        ...
        // استفاده از APIهای داخلی برای بروزرسانی
        const updates = {
            representatives: [],
            salesPartners: [],
            statistics: {
        ...
        for (const update of updatesData) {
            try {
                updates.statistics.processed++;
                
                // تنظیم مقادیر پیش‌فرض برای فیلدهای خالی
                ...
                updates.representatives.push({
                    admin_username: update.admin_username,
                    updates: {
                        name: finalName,
                        ownerName: finalName, // استفاده مشترک از representative_name
                        telegramId: finalTelegramId,
                        sales_associate: update.sales_associate || 'پیش‌فرض'
                    }
                });
                
                // اضافه کردن همکار فروش جدید (در صورت عدم وجود)
                if (update.sales_associate && !updates.salesPartners.find(sp => sp.name === update.sales_associate)) {
                    updates.salesPartners.push({
                        name: update.sales_associate,
                        commission: 3, // 3 درصد پیش‌فرض
                        contactInfo: 'تکمیل بعدی', // مقدار پیش‌فرض
                        isActive: true
                    });
                }
                
                updates.statistics.matched++;
                
            } catch (error) {
                updates.statistics.errors.push({
                    admin_username: update.admin_username,
                    error: error.message
                });
            }
        ...
        console.log(`✅ آمار بروزرسانی:
        - کل پردازش شده: ${updates.statistics.processed}
        - تطبیق یافته: ${updates.statistics.matched}
        - همکاران فروش جدید: ${updates.salesPartners.length}
        - خطاها: ${updates.statistics.errors.length}`);
        
        // ذخیره نتایج
        fs.writeFileSync('representatives-update-result.json', JSON.stringify(updates, null, 2), 'utf8');
        
        console.log('📁 نتایج در فایل representatives-update-result.json ذخیره شد');
        console.log('⚠️ نکته امنیتی: این اسکریپت بدون آسیب به کوپلینگ‌ها و ساختار اتوماسیون طراحی شده است');
        
        return updates;
        
    } catch (error) {
        console.error('❌ خطا در فرایند بروزرسانی:', error);
        throw error;
    }
}
...
if (require.main === module) {
    updateRepresentativesData()
        .then(result => {
            console.log('\n🎉 بروزرسانی با موفقیت تکمیل شد!');
            console.log('📋 جزئیات کامل در فایل JSON ذخیره گردید');
        })
        .catch(error => {
...
module.exports = { updateRepresentativesData, parseRepresentativesData };
